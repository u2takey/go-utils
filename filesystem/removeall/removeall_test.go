...
func TestRemoveAllOneFilesystem(t *testing.T) {
	...
	for _, test := range tests {
		tmpDir, err := temp.CreateTempDir("removeall-" + test.name + "-")
		if err != nil {
		...
		for _, item := range test.items {
			if strings.HasSuffix(item, "/") {
				...
				if err = os.Mkdir(path.Join(tmpDir.Name, item), 0777); err != nil {
					t.Fatalf("error creating %s: %v", item, err)
				}
			} else {
				f, err := os.Create(path.Join(tmpDir.Name, item))
				if err != nil {
					t.Fatalf("error creating %s: %v", item, err)
				}
				f.Close()
		...
		mounter := &fakeMounter{}
		err = RemoveAllOneFilesystem(mounter, tmpDir.Name)
