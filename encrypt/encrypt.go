...
// adapted from https://github.com/henrylee2cn/goutil/
import (
	"crypto/aes"
	"crypto/cipher"
	"crypto/md5"
...
type AESCodec struct {
	key         []byte
	encryptFunc func(ciphertext []byte) []byte
...
func NewAESCodec(key []byte, useBase64 bool, method ...aesMethod) *AESCodec {
	...
	if len(method) > 0 {
		switch method[0] {
		case AESMethodCBC:
			a.encryptFunc = func(ciphertext []byte) []byte {
				return AESCBCEncrypt(key, ciphertext, useBase64)
			}
			a.decryptFunc = func(ciphertext []byte) ([]byte, error) {
				return AESCBCDecrypt(key, ciphertext, useBase64)
			}
		case AESMethodCTR:
			a.encryptFunc = func(ciphertext []byte) []byte {
				return AESCTREncrypt(key, ciphertext, useBase64)
			}
			a.decryptFunc = func(ciphertext []byte) ([]byte, error) {
				return AESCTRDecrypt(key, ciphertext, useBase64)
			}
		}
	}
	...
	if a.encryptFunc == nil {
		a.encryptFunc = func(ciphertext []byte) []byte {
			return AESEncrypt(key, ciphertext, useBase64)
		}
		a.decryptFunc = func(ciphertext []byte) ([]byte, error) {
			return AESDecrypt(key, ciphertext, useBase64)
		}
	}
	return a
}
...
func (a *AESCodec) Encrypt(ciphertext []byte) []byte {
	return a.encryptFunc(ciphertext)
}

func (a *AESCodec) Decrypt(ciphertext []byte) ([]byte, error) {
	return a.decryptFunc(ciphertext)
}
